...
const ProviderAssignService = ({ token }) => {
  ...
  const fetchAssignments = async () => {
    ...
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/v1/providers/data/getProviderHandymanServices`,
        {
  ...
  const fetchDropdownData = async () => {
    ...
    try {
      const [handymenRes, servicesRes] = await Promise.all([
        ...
        axios.get(`${API_BASE_URL}/api/v1/providers/data/getServices`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
  ...
  const handleAssignService = async (e) => {
    ...
    try {
      ...
      await axios.post(
        `${API_BASE_URL}/api/v1/providers/data/assignServiceToHandyman`,
        payload,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const handleDeleteAssignment = async (id) => {
    if (
      !window.confirm("Are you sure you want to remove this assigned service?")
    )
      return;
    try {
      await axios.delete(
        `${API_BASE_URL}/api/v1/providers/data/deleteHandymanService/${id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Allocate official admin services to your active handymen.
          </p>
        </div>
      </div>
        ...
        >
          ...
          <div className="md:col-span-2">
            <button
              type="submit"
              disabled={
                loading || assigning || !selectedHandyman || !selectedService
              }
        ...
        {loadingAssignments ? (
          ...
          <div className="py-8 text-center rounded-2xl bg-slate-800/30 border border-slate-700/50 border-dashed">
            ...
            <p className="text-slate-500 text-xs mt-1">
              Use the form above to assign a service to a handyman.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {assignments.map((assignment) => (
              <div
                ...
                className="bg-slate-800/50 hover:bg-slate-800 border border-slate-700/50 hover:border-emerald-500/30 p-5 rounded-2xl transition-all group flex flex-col items-start"
              >
