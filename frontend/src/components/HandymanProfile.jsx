...
const HandymanProfile = ({ token, onLogout }) => {
    ...
    const fetchData = async () => {
      try {
        const [profileRes, assignmentsRes] = await Promise.all([
          axios.get('http://localhost:3000/api/v1/handymen/data/getHandymanProfile', {
            headers: { Authorization: `Bearer ${token}` }
          }),
          axios.get('http://localhost:3000/api/v1/handymen/data/getHandymanassignedZoneandServices', {
            headers: { Authorization: `Bearer ${token}` }
          })
        ]);
        
        // Handle Profile Data
        const profileData = profileRes.data.handyman || profileRes.data.data || profileRes.data;
        ...
        console.error(err);
        setError(err.response?.data?.message || 'Failed to authenticate data. Session may have expired.');
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen bg-[#0f172a] flex items-center justify-center p-4">
        <div className="flex flex-col items-center">
          ...
          <p className="text-amber-400 font-medium animate-pulse">Loading Workspace...</p>
        </div>
      </div>
    );
  }
  ...
  const hasAssignments = assignments.services.length > 0 || assignments.zones.length > 0;
      ...
      <div className="w-full max-w-4xl relative z-10 flex flex-col gap-6">
        ...
        <div className="flex justify-between items-center backdrop-blur-md bg-slate-900/40 border border-slate-700/50 p-4 md:px-8 md:py-4 rounded-2xl shadow-xl">
           <div className="flex items-center gap-3">
             ...
             <div>
               <h1 className="text-xl font-bold text-white tracking-tight">Handyman Portal</h1>
        ...
        <div className="backdrop-blur-xl bg-slate-900/60 border border-slate-700/50 rounded-3xl overflow-hidden shadow-2xl relative">
              ...
              <div className="px-8 pb-8">
                ...
                <div className="space-y-8">
                  <div>
                    <h2 className="text-3xl font-bold text-white mb-2">{profile?.name || 'Handyman User'}</h2>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-slate-700/50 pt-8">
                      <div className="bg-slate-800/40 rounded-2xl p-6 border border-slate-700/50 flex flex-col justify-center relative overflow-hidden">
                        <div className="absolute top-0 right-0 p-4 opacity-10">
                           <svg className="w-16 h-16" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zM11 7h2v5.41l4.7 4.7-1.41 1.42-5.3-5.3z"></path></svg>
                        </div>
                        <div className="flex items-center gap-3 mb-2">
                           ...
                           <p className="text-sm text-slate-400 font-medium uppercase tracking-wider">Onboarded Since</p>
                        </div>
                      ...
                      <div className="bg-slate-800/40 rounded-2xl p-6 border border-slate-700/50 flex flex-col justify-center relative overflow-hidden">
                        <div className="absolute top-0 right-0 p-4 opacity-10">
                           <svg className="w-16 h-16" fill="currentColor" viewBox="0 0 24 24"><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-7 9h-2V7h-2v5H6v2h2v5h2v-5h2v-2z"></path></svg>
                        </div>
                        <div className="flex items-center gap-3 mb-2">
                           <div className="p-2 bg-orange-500/10 rounded-lg">
                             <svg className="w-5 h-5 text-orange-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg>
                           </div>
                           <p className="text-sm text-slate-400 font-medium uppercase tracking-wider">Assigned Provider</p>
                        </div>
                        <p className="text-slate-200 font-medium text-lg ml-11">
        ...
        <div className="backdrop-blur-xl bg-slate-900/60 border border-slate-700/50 p-6 md:p-8 rounded-3xl shadow-2xl">
           ...
           {!hasAssignments ? (
             <div className="py-12 text-center rounded-2xl bg-slate-800/30 border border-slate-700/50 border-dashed">
               ...
               <p className="text-slate-400 text-sm">No tasks or zones assigned yet.</p>
               <p className="text-slate-500 text-xs mt-1">Waiting for your Provider to delegate responsibilities.</p>
             </div>
           ) : (
             <div className="space-y-8">
               ...
               {assignments.services.length > 0 && (
                 <div>
                    <h4 className="text-md font-semibold text-slate-300 mb-4 border-b border-slate-700/50 pb-2">Active Services</h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {assignments.services.map((svc) => (
                        <div key={svc.id || svc._id} className="bg-slate-800/40 border border-amber-500/20 p-5 rounded-2xl flex items-start gap-4">
                           <div className="p-3 bg-amber-500/10 rounded-xl text-amber-400">
                             <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
                           </div>
                           <div>
                             <h5 className="font-bold text-slate-200">{svc.name}</h5>
               ...
               {assignments.zones.length > 0 && (
                 <div>
                    <h4 className="text-md font-semibold text-slate-300 mb-4 border-b border-slate-700/50 pb-2">Operational Zones</h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                       {assignments.zones.map((zone) => (
                         <div key={zone.id || zone._id} className="bg-slate-800/40 border border-orange-500/20 p-5 rounded-2xl flex items-start gap-4">
                            <div className="p-3 bg-orange-500/10 rounded-xl text-orange-400">
                               <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.242-4.243a8 8 0 1111.314 0z"></path><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                            </div>
                            <div>
                               <h5 className="font-bold text-slate-200">{zone.name}</h5>
                               <p className="text-sm text-slate-400 mt-1">{zone.city}</p>
                            </div>
                         </div>
                       ))}
                    </div>
