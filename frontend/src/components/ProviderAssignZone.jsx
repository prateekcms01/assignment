...
const ProviderAssignZone = ({ token }) => {
  ...
  const fetchAssignments = async () => {
    ...
    try {
      const response = await axios.get('http://localhost:3000/api/v1/providers/data/getProviderHandymanZones', {
  ...
  const fetchDropdownData = async () => {
    ...
    try {
      const [handymenRes, zonesRes] = await Promise.all([
        ...
        axios.get('http://localhost:3000/api/v1/providers/data/getZones', {
          headers: { Authorization: `Bearer ${token}` }
        })
      ]);
  ...
  const handleAssignZone = async (e) => {
    ...
    try {
      ...
      await axios.post('http://localhost:3000/api/v1/providers/data/assignZoneToHandyman', payload, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      setError(err.response?.data?.message || err.message || 'Failed to assign zone.');
    } finally {
      setAssigning(false);
    }
  };
  ...
  const handleDeleteAssignment = async (id) => {
    if (!window.confirm('Are you certain you want to remove this assigned zone?')) return;
    try {
      await axios.delete(`http://localhost:3000/api/v1/providers/data/deleteHandymanZone/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchAssignments();
    } catch (err) {
    ...
    <div className="backdrop-blur-xl bg-slate-900/60 border border-slate-700/50 p-6 md:p-8 rounded-3xl shadow-2xl mt-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Assign operating locations/cities to your handymen staff.</p>
        </div>
      </div>

      <div className="bg-slate-800/40 border border-slate-700/50 rounded-2xl p-6 mb-8">
        {(error || success) && (
          <div className={`p-4 rounded-xl mb-6 text-sm font-medium border ${error ? 'border-red-500/50 bg-red-500/10 text-red-400' : 'border-indigo-500/50 bg-indigo-500/10 text-indigo-400'}`}>
            {error || success}
          </div>
        )}

        <form onSubmit={handleAssignZone} className="grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
          ...
          <div className="md:col-span-5 space-y-2">
            <label className="text-sm font-medium text-slate-300 ml-1">Select Zone (City)</label>
            <div className="relative">
              <select
                value={selectedZone}
              ...
              >
                <option value="">
                  {loading ? 'Loading zones...' : (zones.length === 0 ? 'No zones available' : 'Choose a zone')}
                </option>
                {zones.map((zone) => (
          ...
          <div className="md:col-span-2">
            <button
              type="submit"
              disabled={loading || assigning || !selectedHandyman || !selectedZone}
      ...
      <div className="mt-8 border-t border-slate-700/50 pt-8">
        <h3 className="text-lg font-semibold text-white mb-4">Current Zone Mappings</h3>
        ...
        {loadingAssignments ? (
          ...
          <div className="py-8 text-center rounded-2xl bg-slate-800/30 border border-slate-700/50 border-dashed">
            ...
            <p className="text-slate-500 text-xs mt-1">Use the form above to deploy a handyman to an operational zone.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {assignments.map((assignment) => (
              <div key={assignment.id || assignment._id} className="bg-slate-800/50 hover:bg-slate-800 border border-slate-700/50 hover:border-indigo-500/30 p-5 rounded-2xl transition-all group flex flex-col items-start">
                  ...
                  <div className="flex items-start gap-4 w-full">
                     ...
                     <div className="flex-1">
                        <h4 className="font-semibold text-slate-200 group-hover:text-indigo-400 transition-colors">
                           {assignment.zone_name || assignment.name || 'Assigned Zone'}
                        </h4>
                        <div className="flex items-center gap-1.5 mt-1 text-sm text-slate-400">
                           <svg className="w-4 h-4 text-purple-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                           <span>{assignment.handyman_name || 'Handyman Staff'}</span>
                        </div>
                     </div>
                  </div>
                  ...
                  <div className="w-full border-t border-slate-700/50 mt-4 pt-3 flex justify-between items-center text-xs text-slate-500">
                      ...
                      <div className="flex gap-3 items-center">
                        <span className="px-2 py-0.5 rounded bg-indigo-500/10 text-indigo-400 font-medium tracking-wide">Deployed</span>
                        <button 
