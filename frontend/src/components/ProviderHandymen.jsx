...
const ProviderHandymen = ({ token }) => {
  ...
  const [handymanData, setHandymanData] = useState({ name: '', email: '', password: '' });
  ...
  const fetchHandymen = async () => {
    ...
    try {
      const response = await axios.get('http://localhost:3000/api/v1/providers/data/getProviderHandymen', {
        headers: { Authorization: `Bearer ${token}` }
      });
      // Handle the data array flexibly depending on exactly how it's nested
      setHandymen(response.data.handymen || response.data.data || response.data || []);
    } catch (err) {
  ...
  const handleCreateHandyman = async (e) => {
    ...
    try {
      await axios.post('http://localhost:3000/api/v1/providers/data/createHandyman', handymanData, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      setHandymanError(err.response?.data?.message || err.message || 'Failed to create handyman.');
    } finally {
    ...
    <>
      <div className="backdrop-blur-xl bg-slate-900/60 border border-slate-700/50 p-6 md:p-8 rounded-3xl shadow-2xl">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">Manage all handymen registered under your account.</p>
          </div>
          <button
        ...
        {loadingHandymen ? (
          ...
          <div className="py-12 text-center rounded-2xl bg-slate-800/30 border border-slate-700/50 border-dashed">
            ...
            <p className="text-slate-500 text-xs mt-1">Click "Add Handyman" to register a team member.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {handymen.map((handyman) => (
              <div key={handyman.id || handyman._id} className="bg-slate-800/50 hover:bg-slate-800 border border-slate-700/50 hover:border-teal-500/30 p-5 rounded-2xl transition-all group flex flex-col">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400 truncate w-32 md:w-40" title={handyman.email}>
                        {handyman.email}
                      </p>
                    </div>
                  </div>
      ...
      {showHandymanModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-sm" onClick={() => setShowHandymanModal(false)}></div>
          <div className="relative bg-slate-900 border border-slate-700 p-6 md:p-8 rounded-3xl shadow-2xl w-full max-w-md animate-in fade-in zoom-in-95 duration-200">
            <h3 className="text-2xl font-bold text-white mb-2">Register Handyman</h3>
            <p className="text-sm text-slate-400 mb-6">Create a sub-account for your handyman staff.</p>
            
            {(handymanError || handymanSuccess) && (
              <div className={`p-3 rounded-xl mb-6 text-sm font-medium border ${handymanError ? 'border-red-500/50 bg-red-500/10 text-red-400' : 'border-teal-500/50 bg-teal-500/10 text-teal-400'}`}>
                {handymanError || handymanSuccess}
              </div>
            )}
            ...
            <form onSubmit={handleCreateHandyman} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  onChange={(e) => setHandymanData({ ...handymanData, name: e.target.value })}
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  onChange={(e) => setHandymanData({ ...handymanData, email: e.target.value })}
                  className="w-full px-4 py-3 rounded-xl bg-slate-800 border border-slate-700 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-teal-500/50 focus:border-teal-500/50 transition-all"
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  onChange={(e) => setHandymanData({ ...handymanData, password: e.target.value })}
                  className="w-full px-4 py-3 rounded-xl bg-slate-800 border border-slate-700 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-teal-500/50 focus:border-teal-500/50 transition-all"
                  placeholder="••••••"
                />
              </div>
